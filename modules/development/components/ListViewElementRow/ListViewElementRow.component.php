...
function render ($props) {
    ...
    if (!$hasTechnologyRequirementMet && $CurrentUser['settings_ExpandedBuildView'] == 0) {
        return [
            'componentHTML' => '',
        ];
    }
    ...
    if ($hasTechnologyRequirementMet) {
        ...
        if (!Elements\isPremiumStructure($elementID)) {
            ...
            $subcomponentUpgradeResourcesLeftoverRowHTML = (
                ...
                "<b style=\"color: {$resourceDiffRestColor};\"> " . prettyNumber($resourceDiff) . "</b>" .
                "</font>"
            );
        }
        ...
        if ($isInQueue && $elementQueueLevelModifier != 0) {
            $subcomponentUpgradeLevelHeaderRowHTML = (
                "<b>[{$_Lang['level']}: {$elementNextLevelToQueue}]</b><br/>"
            );
        }
    }
